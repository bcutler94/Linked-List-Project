...
class LinkedList {
    ...
    insert(index, val) {
        // if the index is out of the bounds of the ll then immediately return false
        if (index < 0 || index >= this.length) return false;
        // if the index is the length we know to add to tail
        if (index === this.length) return !!this.addToTail(val);
        // if the index is zero we know to add to head
        if (index === 0) return !!this.addToHead(val);
        // initialize new instance of node to be inserted
        const newNode = new Node(val);
        // call get method that returns the node before the given index since we
        // need to change the pointer
        const prevNode = this.get(index - 1);
        // save the node's pointer to variable so not overwritten by node to be inserted
        const temp = prevNode.next;
        // set the node's pointer to the node to be inserted
        prevNode.next = newNode;
        // set the new node's pointer to old pointer
        newNode.next = temp;
        this.length++
        return true;
    }
    ...
    remove(index) {
        // if the index is out of bounds then immediately return false
        if (index < 0 || index >= this.length) return undefined;
        // if the index is 0 we can call our removehead method
        if (index === 0) return !!this.removeHead();
        // if the index is the length we can call remove tail method
        if (index === this.length) return !!this.addToHead();
        // grab the node at prev index to be removed
        const prev = this.get(index - 1);
        // save the node's next to variable so not overwritten by removal
        const prevNext = prev.next;
        // set the node at new current index's pointer to its pointer of pointer
